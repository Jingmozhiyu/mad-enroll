...
export function SearchOverlay({
    ...
    return createPortal(
        <div
            className="overlay-backdrop fixed inset-0 z-50 flex items-start justify-center px-4 py-6 backdrop-blur-sm md:py-10">
            <div className="glass-card relative max-h-[90vh] w-full max-w-5xl overflow-hidden">
                <div
                    className="subtle-panel-divider flex items-start justify-between gap-4 border-b px-6 py-4 md:px-8 md:py-5">
                    <div>
                        <h2 className="text-2xl font-semibold text-[var(--color-ink)] md:text-3xl mt-2">
                            Search Courses
                        </h2>
                    </div>
                ...
                <div className="flex max-h-[calc(90vh-6rem)] flex-col gap-4 overflow-y-auto px-6 py-4 md:px-8 md:py-5">
                    <form
              ...
              <span className="text-sm font-bold text-[var(--color-ink-soft)]">
                ...
                {searchStage === 'courses'
                    ...
                    {searchStage === 'sections' && selectedCourse ? (
                        <div
                            className="flex flex-wrap items-center justify-between gap-3 border-y border-[var(--surface-divider)] py-4">
                            <div>
                    ...
                    >
                        {searchStage === 'courses' ? (
                            courseResults.length === 0 ? (
                                <div
                                    className="surface-panel-dashed rounded-[14px] px-5 py-9 text-center text-sm text-[var(--color-ink-soft)]">
                                    ...
                                    <div className="grid gap-4 md:grid-cols-2">
                                        {courseResults.map((course) => (
                                            <article
                                                ...
                                                className="surface-panel rounded-[14px] px-4 py-4 transition-colors hover:bg-[var(--surface-hover)]"
                                            >
                            ...
                            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                                {sectionResults.map((section) => {
                                    ...
                                    return (
                                        <article
                                            key={section.docId}
                                            className="surface-panel grid gap-4 rounded-[14px] px-4 py-4 transition-colors hover:bg-[var(--surface-hover)]"
                                        >
                                            <div className="grid gap-4">
                                                ...
                                                <div
                                                    className="space-y-3 text-sm leading-6 text-[var(--color-ink-soft)]">
                                                    {meetingDetails.length > 0 ? (
                                                        <div className="space-y-2">
                                                            {meetingDetails.map((meeting, index) => (
                                                                <div key={`${section.docId}-meeting-${index}`}
                                                                     className="space-y-0.5">
                                                                    <p>{meeting.schedule}</p>
                                                                    {meeting.location ?
                                                                        <p>{meeting.location}</p> : null}
                                                                </div>
                                                            ))}
                                                        </div>
                                                    ) : null}
